...
class _NotificationDetailPageState extends State<NotificationDetailPage> {
  ...
  Future<void> _markAsReadIfNeeded() async {
    if (!widget.notification.isRead && !_marking) {
      ...
      try {
        await context.read<ApiService>().markNotificationRead(
                      ...
                      onPressed: () async {
                        try {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text('Không mở được công việc: $e'),
                            ),
                          );
                        }
                      },
                  ...
                  if (n.refType == 'event')
                    OutlinedButton.icon(
                      ...
                      onPressed: () async {
                        await Navigator.of(context).push(
                          MaterialPageRoute(
                            builder: (_) => EventDetailPage(eventId: n.refId!),
                          ),
                        );
                      },
                    ),
                  if (n.refType == 'event') ...[
                    Builder(
                      builder: (ctx) {
                        ...
                        final ev =
                            api.events.where((e) => e.id == n.refId).isNotEmpty
                                ...
                                onPressed: () async {
                                  try {
                                    ...
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(
                                        content: Text(
                                          'Đã chấp thuận yêu cầu thay đổi',
                                        ),
                                      ),
                                    );
                                  } catch (e) {
