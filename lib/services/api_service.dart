...
class ApiService extends ChangeNotifier {
  ...
  List<ScheduleItemModel> upcomingSchedule = [];
  ...
  Future<void> fetchUpcomingSchedule({DateTime? from, DateTime? to}) async {
    final now = DateTime.now();
    final start = from ?? now;
    final end = to ?? now.add(const Duration(days: 14));
    ...
    final list = <ScheduleItemModel>[];
    list.addAll(taskList.map((e) => ScheduleItemModel.fromJson(e)));
    list.addAll(eventList.map((e) => ScheduleItemModel.fromJson(e)));
    list.sort((a, b) {
      final ta = a.startTime ?? a.endTime ?? DateTime.now();
      final tb = b.startTime ?? b.endTime ?? DateTime.now();
  ...
  Future<bool> login(String email, String password) async {
    ...
    // Check for error responses (401, 403)
  ...
  // Public: request password reset (admin-mediated). Always returns true if request sent.
  Future<bool> requestPasswordReset(String email) async {
    await _dio.post('/api/auth/forgot-password', data: {'email': email});
    // API returns generic message; no state change needed.
    return true;
  }
  ...
  Future<UserModel> adminUnlockUser(String id) async {
    final res = await _dio.post('/api/users/$id/unlock');
  ...
  // Pre-check user business trip conflict before creating & assigning task
  Future<Map<String, dynamic>> checkUserBusinessTripConflict({
    required String userId,
    required DateTime start,
    DateTime? end,
  }) async {
    final res = await _dio.get(
      '/api/users/$userId/business-trip-conflict',
      queryParameters: {
